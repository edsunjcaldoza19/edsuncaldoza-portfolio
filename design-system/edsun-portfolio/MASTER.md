...
- Project cards: 4:3 media, project index, category, outcome, and a clearly labeled Behance project link. In multi-column galleries, use equal-height copy areas, reserve two title lines, and anchor actions to the bottom for clean row alignment. Remove reserved title height in single-column layouts. Future portfolio slots are visibly labeled as coming soon, remain inert, and never generate routes.
