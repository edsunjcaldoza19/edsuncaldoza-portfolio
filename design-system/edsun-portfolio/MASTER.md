...
- Project categories: four visible homepage rows place the category name and description on the left and the completed-project count on the right. Each row is an opaque surface card with a 1px border, 12px radius, fluid 24–48px padding, and a restrained theme-aware shadow. Omit repeated category numbers and generic labels. Each row exposes its work immediately without a modal.
