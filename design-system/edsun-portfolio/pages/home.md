...
- Use Motion only on the inner category card while native CSS controls the outer sticky wrapper. Track each incoming card from `start 90%` to `start 25%` with content-size updates, mapping `y` from `48px` to `0` and scale from `.985` to `1`. Keep opacity fixed at `1` throughout the entrance. Use direct scroll progress without springs or scroll interception. Below `768px`, keep normal document flow. Under reduced motion, remove both the transforms and sticky stacking.
