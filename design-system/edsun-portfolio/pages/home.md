...
- Wrap each complete category row in an opaque surface card with a 1px border, 12px radius, fluid `24–48px` padding, and a restrained shadow. At `768px` and above, make the outer category wrapper sticky at `calc(var(--nav-height) + 24px + var(--stack-offset))`, increase the offset by `16px` per category, and increase stacking order with the category index. Keep the main Selected Work heading and Behance action outside this stack.
...
- Use Motion only on the inner category card while native CSS controls the outer sticky wrapper. Track each incoming card from `start 90%` to `start 25%` with content-size updates, mapping `y` from `48px` to `0`, scale from `.985` to `1`, and opacity from `.78` to `1`. Use direct scroll progress without springs or scroll interception. Below `768px`, keep normal document flow. Under reduced motion, remove both the transforms and sticky stacking.
