...
- Present Graphic Design, Webinar Presentations, Web Design, and Video Editing as four visible horizontal project rows beneath “Get to know me through my work.” Put the category number, name, and description on the left of each row header and the project count in a compact right-aligned area.
- Show three equal cards when a desktop category has exactly three projects. For longer categories, show three full cards plus part of the next card, with a soft edge fade that communicates additional content. Reduce the visible measure to roughly two cards plus a partial card on tablet and one card plus a partial card on mobile.
- Keep each project lane natively scrollable, keyboard focusable, touch swipeable, and contained within the page. Use proximity scroll snapping. Add paired 48px circular Previous and Next controls only when a category contains more than three projects, move exactly one card per activation, and expose clear disabled states at both ends. Remove modal, backdrop, focus-trap, and document scroll-lock behavior entirely.
