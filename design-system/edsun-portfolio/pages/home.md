...
- Present Graphic Design, Web Design, Webinar Presentations, and Video Editing in that order as four visible horizontal project rows beneath “Get to know me through my work.” Put the category name and description on the left of each row header and the project count in a compact right-aligned area. Do not repeat a category number or generic “Project category” label.
- Show three equal cards when a desktop category has exactly three projects. For longer desktop categories, show three full cards plus roughly 40% of the next card. From 768px to 1024px, show two complete cards plus roughly 30% of the third using a `2.3` viewing ratio. Below 768px, show one complete card plus roughly 20% of the second using a `1.2` viewing ratio. Keep the remaining projects available through dragging, swiping, keyboard controls, or navigation buttons.
- Use Embla Carousel with start alignment, no loop, trimmed end snaps, and one-card navigation. Keep each project lane keyboard focusable, touch and mouse draggable, and contained within the page. Render 48px circular Previous and Next controls with Lucide chevrons at the vertical center of the image row whenever Embla reports actual overflow. Hide Previous completely at the start and expose a clear disabled Next state at the end. Use immediate movement under reduced motion.
- Apply standard and `-webkit-` masks to the complete carousel viewport. Fade the right edge while more projects remain, the left edge after moving forward, and both edges between the beginning and end. Keep controls outside the masked viewport. Provide one-pixel-extended gradient overlays as a no-mask fallback so no uncovered media sliver appears at either boundary.
- Wrap each complete category row in a fully opaque surface card with a 1px border, 12px radius, fluid `24–48px` padding, and a very soft top-edge shadow. Avoid broad shadows that compound as cards stack. At `768px` and above, make the outer category wrapper sticky at `calc(var(--nav-height) + 24px + var(--stack-offset))`, increase the offset by `16px` per category, and increase stacking order with the category index. Keep the main Selected Work heading and Behance action outside this stack.
- Before Embla initializes, keep the lane natively horizontally scrollable with proximity snap as progressive enhancement. Remove modal, backdrop, focus-trap, document scroll-lock, and the previous manual scroll-measurement controller entirely.
...
- Use Motion only on the inner category card while native CSS controls the outer sticky wrapper. Track each incoming card from `start 90%` to `start 25%` with content-size updates, mapping `y` from `48px` to `0`, scale from `.985` to `1`, and alternating rotation from `-1.25deg` or `1.25deg` to a level `0deg` final state. Begin with a left tilt for Graphic Design, rotate around the top center, and keep opacity fixed at `1` throughout the entrance. Each incoming category settles above earlier cards through the indexed sticky wrapper. Use direct scroll progress without springs or scroll interception. Below `768px`, keep normal document flow. Under reduced motion, remove rotation, scale, vertical movement, and sticky stacking.
